...
Engine::Engine(const PBEConfig& cfg):config_(cfg),beauty_(default_beauty_params()),auto_(default_auto_settings()){reset_advanced_color(advanced_color_);std::memset(&ai_provider_,0,sizeof(ai_provider_));}
void Engine::set_error(const char* m){last_error_=m?m:"";}
PBEStatus Engine::reset(){beauty_=default_beauty_params();auto_=default_auto_settings();face_=nullptr;detected_face_={};reset_advanced_color(advanced_color_);has_ai_provider_=false;std::memset(&ai_provider_,0,sizeof(ai_provider_));input_rgba_.pixels.clear();output_rgba_.pixels.clear();last_error_.clear();return PBE_OK;}
...
PBEStatus s=pipeline_.render(input_rgba_,output_rgba_,beauty_,face_,advanced_color_,config_.quality,config_.thread_count);if(s!=PBE_OK){set_error("Pipeline render failed.");return s;}return export_image(output_rgba_,output);}
